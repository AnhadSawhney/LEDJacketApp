...
public class GrabAudio {
    ...
    private final String TAG = "GRAB_AUDIO";
    ...
    public GrabAudio(int type, int size, int audioSession) {
        ...
        if(!mUseSnoop) {
            ...
            Log.v(TAG, "Capture size range: " + Integer.toString(range[0]) + "-" + Integer.toString(range[1]));
            mType = type;
    ...
    public int[] getFormattedData(int num, int den) { // numerator denominator
        if (num == 0)
            num += 1;
        if(!mUseSnoop) {
            captureData();
            if (mType == PCM_TYPE) {
                for (int i = 0; i < mFormattedData.length; i++) {
                    int tmp = ((int)mRawData[i] & 0xFF) - 128; // unsigned 8 bit pcm datta
    ...
    public void captureData() {
        ...
        try {
            ...
            if (status != Visualizer.SUCCESS) {
                ...
                if (mType == PCM_TYPE)
                    Arrays.fill(mRawData, (byte)0x80); //128
                else
